 ...
 *  org.bukkit.craftbukkit.v1_19_R1.event.CraftEventFactory
...
public final class Wrapper1_19_R1
implements VersionWrapper {
    ...
    public void setActiveContainerDefault(Player player) {
        this.toNMS((Player)player).bU = this.toNMS((Player)player).bT;
    }

    @Override
    public void setActiveContainer(Player player, VersionWrapper.AnvilContainerWrapper container) {
        this.toNMS((Player)player).bU = (Container)container;
    }
    ...
    public VersionWrapper.AnvilContainerWrapper newContainerAnvil(Player player, Object title) {
        if (this.IS_ONE_NINETEEN_ONE) {
            return new AnvilContainer1_19_1_R1(player, this.getRealNextContainerId(player), (IChatBaseComponent)title);
        }
    ...
    implements VersionWrapper.AnvilContainerWrapper {
        public AnvilContainer(Player player, int containerId, IChatBaseComponent guiTitle) {
            super(containerId, ((CraftPlayer)player).getHandle().fB(), ContainerAccess.a((World)((CraftWorld)player.getWorld()).getHandle(), (BlockPosition)new BlockPosition(0, 0, 0)));
